...
(function () {
  ...
  var Router = (function () {
    ...
    return { showDashboard: showDashboard, open: open, setTag: setTag };
  })();
  ...
  window.APP = {
    ...
    open: Router.open, dashboard: Router.showDashboard, store: Store,
  ...
  function renderDashboard() {
    ...
    g1.appendChild(card({
      ...
      desc: "Where " + EXAM + " sits on the career ladder for AI-security practitioners, and the SOC analyst, AI security engineer, and GRC roles it supports.",
      foot: "Specialty · AI security", cta: "Read",
    ...
    g5.appendChild(card({
      ...
      desc: "Plan your certification journey: browse 36 industry certifications across 12 vendors with live filters, costs, full exam blueprints, DoD 8140 mapping, and salary and role insights.",
      foot: "36 certifications · 12 vendors", cta: "Explore",
  ...
  function computeReadiness() {
    ...
    var dims = [
      ...
      { key: "tax", label: "Taxonomy mapping", weight: 0.8, value: taxVal, has: txKeys.length > 0, kind: "score", detail: txKeys.length ? (txKeys.length + "/" + (taxTotal || 15) + " activities, avg " + taxVal + "%") : "not attempted" }
    ];
    ...
    return { dims: dims, overall: overall, anyActivity: anyActivity, domains: domains, bestMock: bestMock, fa: fa, pbqDone: pbqDone, pbqTotal: pbqTotal, labDone: labDone, labTotal: labTotal || 20, mocks: mocks.length, quizzes: quizzes.length, taxAttempted: txKeys.length, taxTotal: taxTotal || 15 };
  }
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.open(readerView("Domain " + id, d.title, CODE + " Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
